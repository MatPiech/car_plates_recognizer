...
class ExtractPlate:
    ...
    def maximizeContrast(self, img_gray: np.ndarray) -> np.ndarray:
        ...
        img_gray_plus_TopHat_minus_BlackHat = cv2.subtract(
            img_gray_plus_TopHat, img_BlackHat)
    ...
    def preprocess(self, img: np.ndarray) -> np.ndarray:
        ...
        img_thresh = cv2.adaptiveThreshold(
            img_blurred, 255.0, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 19, 9)
    ...
    def findMatchingChars(self, possible_character: tuple, list_of_characters: list) -> list:
        ...
        for possible_matching_characters in list_of_characters:
            ...
            distance_between_chars = np.sqrt(
            ...
            if (distance_between_chars < (diagonal * 5.0) and angle_between_chars < 12.0 and
                ...
                list_of_matching_characters.append(
                    possible_matching_characters)
    ...
    def findListOfListsOfMatchingChars(self, list_of_possible_characters: list) -> list:
        ...
        for possible_character in list_of_possible_characters:
            list_of_matching_characters = self.findMatchingChars(
                possible_character, list_of_possible_characters)
            ...
            list_of_possible_characters_with_current_matches_removed = list(
                set(list_of_possible_characters) - set(list_of_matching_characters))

            recursivelist_of_lists_of_matching_chars = self.findListOfListsOfMatchingChars(
                list_of_possible_characters_with_current_matches_removed)
            ...
            for recursivelist_of_matching_characters in recursivelist_of_lists_of_matching_chars:
                list_of_lists_of_matching_chars.append(
                    recursivelist_of_matching_characters)
    ...
    def extractPlate(self, img: np.ndarray, list_of_matching_characters: list) -> np.ndarray:
        ...
        img_cropped = cv2.getRectSubPix(
            img_rotated, (plate_width, plate_height), plate_center)

        return img_cropped

    def detectPlate(self, img: np.ndarray) -> list:
        ...
        list_of_possible_characteracters = self.findpossible_characteracters(
            img_thresh)

        list_of_lists_of_matching_characters = self.findListOfListsOfMatchingChars(
            list_of_possible_characteracters)
    ...
    def detectPlateSimple(self, img: np.ndarray, d: int = 11, th1: int = 10, epsilon: float = 0.02, scale: float = 0.5, h: int = 80, w: int = 400) -> list:
